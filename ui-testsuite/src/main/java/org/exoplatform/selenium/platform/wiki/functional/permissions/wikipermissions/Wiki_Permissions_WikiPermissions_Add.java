...
public class Wiki_Permissions_WikiPermissions_Add extends BasicAction {
	...
	public void test09_AddPermissionForGroupBySelectingDirectly() {
		...
		magAcc.userSignIn(ManageAccount.userType.ADMIN);
		goToWiki();
		resetDataWikiSpacePermission(ELEMENT_PAGE1, "*:/developers");
	}
	...
	/**
	 *Add a blank wiki page
	 *Delete a space permission for Group: Any
	 *Add a space permission
	 *<Edit a space permission 
	 */
	public void prepareDataWikiSpacePermissionV1(String pageName, int option, String[] userGroupToAddPermission, String userGroupToEditPermission){
		...
		assert !waitForAndGetElement(ELEMENT_EDIT_PAGE_PERMISSIONS.replace("${user}", userGroupToEditPermission), DEFAULT_TIMEOUT,1,2).isSelected();
		assert !waitForAndGetElement(ELEMENT_ADMIN_SPACE_CHECK.replace("{$user}", userGroupToEditPermission), DEFAULT_TIMEOUT,1,2).isSelected();
		assert !waitForAndGetElement(ELEMENT_ADMIN_PAGE_CHECK.replace("{$user}", userGroupToEditPermission), DEFAULT_TIMEOUT,1,2).isSelected();
		editSpacePermission(userGroupToEditPermission, true, true, true, true, 2);
	}

	/**
	 *Add a blank wiki page
	 *Go to Space Permission
	 *Delete Space Permission Group: Any
	 *Select an User Permission 
	 *Edit Space Permission 
	 */
