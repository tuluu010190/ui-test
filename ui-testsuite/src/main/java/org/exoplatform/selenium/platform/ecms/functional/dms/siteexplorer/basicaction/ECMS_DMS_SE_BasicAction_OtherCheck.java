...
public class ECMS_DMS_SE_BasicAction_OtherCheck  extends EcmsBase {
	...
	@Test
	public void test39_40_AddRemoveThumbnailImageForNode(){
		...
		click(LIST_VIEW);
		waitForElementNotPresent(OVERLOAD_THUMBNAIL);
		info("Upload file");
		...
		click(ICON_VIEW);
		waitForElementPresent(OVERLOAD_THUMBNAIL);
		info("Upload thumbnail");
		uploadThumb(DATA_UPLOAD_FILE);
		...
		assert isElementPresent(OVERLOAD_THUMBNAIL):"Can not see";
		click(OVERLOAD_THUMBNAIL);
		info("-------------");
		assert isElementPresent(By.id("UIPopupWindow")):"Can not see popup";
